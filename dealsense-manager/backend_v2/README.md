...
- HTTP endpoint: `GET /health` (includes database connectivity check)
...
The application uses `config.yaml` as the primary configuration source. Copy the provided `config.yaml` and modify it according to your environment:

```bash
cp config.yaml config.local.yaml  # For local development
```

**Configuration:**
- **`config.yaml`** is the single source of truth for all configuration
- No environment variable overrides are supported
- For different environments, create different `config.yaml` files
...
- **`internal/services/`** - Business logic and agent management
- **`internal/models/`** - Data structures
- **`config.yaml`** - Configuration file (primary config source)
